...
function _update()
	if (btn(:flれそche_bas:)) p.angle += p.rotate_speed
	if (btn(:flれそche_haut:)) p.angle -= p.rotate_speed
	if (btn(:flれそche_droite:)) move_player(1)
	if (btn(:flれそche_gauche:)) move_player(-1)
end
function _draw()
	cls()
	minimap()
